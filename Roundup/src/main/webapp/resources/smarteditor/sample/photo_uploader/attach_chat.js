	...
	var nMaxTotalImageSize = 10*1024*1024;
	var nMaxImageCount = 1;
    ...
    function drop(ev) {
		...
		if (nImageFileCount > 1){
			alert("최대 1장까지만 등록할 수 있습니다.");
			return;
		}
    ...
    function html5Upload() {	
    	...
    	sUploadURL= '../../../../../rup/chatting/chatUpload.do'; 	//upload URL
    ...
    function makeArrayFromString(sResString){
    	...
    	if(aResult.length == nImageFileCount){
    		...
    		aResult = null;
    		alert("업로드완료! 이미지전송을 눌러주세요.");
    		window.opener.document.successUpload.chatUpload.value="업로드성공";
    		window.close();
    	}
    }
